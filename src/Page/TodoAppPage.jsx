...
function TodoAppPage() {
  ...
  useEffect(()=>{
    if(todoList.length>0){localStorage.setItem('todokey',JSON.stringify(todoList));
  }   
  },[todoList])
  ...
  const onSaveEdit = (event) => {
    event.preventDefault();
    if (editedTodo === "") {
      toast.warn("Please provide valid todo", {
        position: "top-center",
        theme: "colored",
        autoClose: 1000,
      });
  ...
  const clearTodoList = (event) => {
    ...
    setTodoList([]);
    localStorage.clear()
    toast.success("TodoList is deleted", {
        position: "top-center",
        theme: "colored",
        autoClose: 1000,
      })
              ...
              {todoFilterPage.map((eachElement) => {
                return (
                  ...
                  >
                    <div>
                      {editedTodoId === eachElement.id ? (
                        ...
                        <>
                          ...
                          />
                          <label
                            className="todo-list-inputs"
                            htmlFor={eachElement.id}
                          >
                            {eachElement.title}
                          </label>
                        </>
                    ...
                    <div>
                      {eachElement.status && (
                        <label className="todo-checkbox">completed</label>
                      )}
                      <button
                        type="button"
